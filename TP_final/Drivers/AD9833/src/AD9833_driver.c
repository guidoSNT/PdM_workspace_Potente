 ...
 * 2^28 / fmclk.
 */
static inline uint32_t get_freq_reg(uint32_t freq) { return ((uint64_t) freq << AD9833_FREQ_REG_LEN) / (device.fmclk * AD9833_MHZ_IN_dHZ) + AD9833_CONVERSION_OFFSET; }

ad9833_st_t ad9833_init(uint8_t mclk) {
    ad9833_st_t res;
    // uartInit();
    if (mclk == AD9833_FMCLK_MIN_FREQ || mclk > AD9833_FMCLK_MAX_FREQ) return AD_BAD_ARGUMENT;

    device.fmclk = mclk;

    if (spi_init() != AD_SPI_OK) return AD_NOT_INIT;

    res = ad9833_set_control(AD9833_CTRL_RESET);
    if (res == AD_NO_RESPONSE) return AD_FAIL;
...
ad9833_st_t ad9833_set_form(ad9833_wf_t form) {
    if (device.init == false) return AD_NOT_INIT;
    ...
    switch (form) {
        ...
        case SQUARE_DIV:
            reg_temp = (device.ctrl_reg & ~AD9833_CTRL_MODE) | AD9833_CTRL_OPBITEN | AD9833_CTRL_DIV2;
            break;

        default:
            return AD_BAD_ARGUMENT;
    }
    res = ad9833_set_control(reg_temp);
...
ad9833_st_t ad9833_set_phase(uint16_t phase, reg_t reg) {
    ...
    if (AD_SPI_OK != write_word(reg_temp | phase_reg)) return AD_FAIL;

    if (reg == REG_0) device.phase0 = phase_reg;
    else device.phase1 = phase_reg;
...
ad9833_st_t ad9833_set_freq(uint32_t freq, reg_t reg) {
    ...
    if (freq > device.fmclk * AD9833_MHZ_IN_dHZ / 2) return AD_BAD_ARGUMENT;
    ...
    if (AD_SPI_OK != write_word(reg_temp | (uint16_t) (freq & AD9833_FREQ_14BIT_MASK))) return AD_FAIL;

    if (full_tx) {
        if (AD_SPI_OK != write_word(reg_temp | (uint16_t) ((freq >> AD9833_FREQ_14BIT_SHIFT) & AD9833_FREQ_14BIT_MASK))) return AD_FAIL;
    }

    if (reg == REG_0) device.freq0 = freq_reg;
    else device.freq1 = freq_reg;
...
ad9833_st_t ad9833_set_control(uint16_t control) {
    ...
    device.ctrl_reg = control & AD9833_FREQ_14BIT_MASK;
    return (write_word(device.ctrl_reg) == AD_SPI_OK) ? AD_OK : AD_FAIL;
}
