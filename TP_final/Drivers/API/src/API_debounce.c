/**
 * @file API_debounce.c
 * @brief Source file to fsm to debounce buttons.
 */
    ...
    switch (dev->cur_state) {
        case BUTTON_UP:
            if (btn_state == BTN_PIN_LOW) {
                if (delayIsRunning(&dev->dly)) delayStop(&dev->dly);
        ...
        case BUTTON_FALLING:
            if (delayRead(&dev->dly)) {
                ...
                if (dev->cur_state == BUTTON_DOWN) dev->pressed = true;
            }
            break;
        ...
        case BUTTON_RAISING:
            if (delayRead(&dev->dly)) { dev->cur_state = (btn_state == BTN_PIN_HIGH) ? BUTTON_UP : BUTTON_DOWN; }
            break;
